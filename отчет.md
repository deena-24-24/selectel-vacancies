# Отчёт по отладке приложения Исмагиловой Дины Маратовны
...
### 8. Отсутствие валидации уникальности при обновлении
*   **Проблема:** При обновлении вакансии сервер не проверял, занят ли новый `external_id` другой записью. При коллизии PostgreSQL выбрасывал `UniqueViolation`, а сервер падал с `500 Internal Server Error`. Можно рассмотреть возможность исключения поля `external_id` из `VacancyUpdate`, чтобы повысить целостность данных.
...
**Итог:** Приложение работает стабильно, база данных наполняется актуальными вакансиями в фоновом режиме каждые 5 минут, API обрабатывает запросы согласно стандартам REST.